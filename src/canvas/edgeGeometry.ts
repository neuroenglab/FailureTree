...
export function computeEdgeGeometry(edge: FlowEdge, nodes: FlowNode[]): EdgeGeometry | null {
  ...
  const from = isJunction(source) ? boxCenter(sourceBox) : anchor(sourceBox, fromSide);
  const to = isJunction(target) ? boxCenter(targetBox) : anchor(targetBox, toSide);

  const obstacles = nodes
    .filter((n) => n.id !== source.id && n.id !== target.id && !isJunction(n))
    .map(nodeBox);

  return routeCubic(from, fromSide, to, toSide, obstacles);
}
